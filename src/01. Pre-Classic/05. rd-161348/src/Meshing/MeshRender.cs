...
public class MeshRender : IDisposable
{
    ...
    private void DrawElementsLine(Shader shader)
    {
            ...
            Game.ShadingMode == ShadingMode.Shaded_Wireframe)
        {
            ...
            if (_hasColor) _gl.DisableVertexAttribArray(1);
            if (_hasTexture) _gl.DisableVertexAttribArray(2);
            ...
            if (_hasColor) _gl.EnableVertexAttribArray(1);
            if (_hasTexture) _gl.EnableVertexAttribArray(2);
    ...
    {
        _hasColor = colors.Length > 0;
        _hasTexture = texCoords.Length > 0;
