...
public class BlockInteraction
{
    ...
    public void Update()
    {
        _paintTexture = GuiSelectedBlock.PaintTexture;

        if (RayCast())
        {
            RenderHit(_hitResult, _editMode, _paintTexture);
    ...
    private void HandleMouseClick()
    {
        ...
        else
        {
            ...
            AABB bounds = Block.Blocks[_paintTexture].GetBounds(position);

            if (bounds == null || IsFree(bounds))
            {
                _level.SetBlock(position, _paintTexture);
            }            
        }
    }
    ...
    {
        if (_camera.Bounds.Intersects(bounds))
        {
            return false;
        }
    ...
    private void RenderHit(HitResult h, int mode, int blockType)
    {
        ...
        if (mode == 0)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                Block.Rock.AddFace(_mesh, new Vector3Int(h.X, h.Y, h.Z), (BlockFace)i);
            }
        }
        else
        {
            float br = (float)Mathf.Sin(Environment.TickCount / 100.0f) * 0.2f + 0.8f;
            float alpha = Mathf.Sin(Environment.TickCount / 200.0f) * 0.2f + 0.5f;
            _color = new Color(br, br, br, alpha);

            int x = h.X;
            int y = h.Y;
            int z = h.Z;

            if (h.F == (int)BlockFace.NegativeX)
            {
                x--;
            }
            if (h.F == (int)BlockFace.PositiveX)
            {
                x++;
            }
            if (h.F == (int)BlockFace.NegativeY)
            {
                y--;
            }
            if (h.F == (int)BlockFace.PositiveY)
            {
                y++;
            }
            if (h.F == (int)BlockFace.NegativeZ)
            {
                z--;
            }
            if (h.F == (int)BlockFace.PositiveZ)
            {
                z++;
            }
            ...
            for (int i = 0; i < 6; i++)
            {
                Block.Blocks[blockType].AddFaceWithTexture(_mesh, new Vector3Int(x, y, z), (BlockFace)i);
            }
            */

            _mesh.NoColor();
            Block.Blocks[blockType].Init(_mesh, _level, new Vector3Int(x, y, z));
        }        
