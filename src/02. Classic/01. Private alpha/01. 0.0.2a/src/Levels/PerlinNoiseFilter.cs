...
public class PerlinNoiseFilter
{
	...
	public int[] read(int width, int height)
    {
		...
		for(y = 0; y < height; y += result)
        {
            ...
            {
				tmp[x + y * width] = (random.Next(256) - 128) * this._fuzz;
			}
		}
		...
		for(result = width >> level; result > 1; result /= 2)
        {
			...
			for(y1 = 0; y1 < height; y1 += result)
            {
				for(x1 = 0; x1 < width; x1 += result)
                {
					...
					ml = (c + d + r + mu) / 4 + random.Next(y * 2) - y;
			...
			for(y1 = 0; y1 < height; y1 += result)
            {
				for(x1 = 0; x1 < width; x1 += result)
                {
					...
					int u = (c + r + m + mu) / 4 + random.Next(y * 2) - y;
					int l = (c + d + m + ml) / 4 + random.Next(y * 2) - y;
					tmp[x1 + x + y1 * width] = u;
					tmp[x1 + (y1 + x) * width] = l;
				}
			}
